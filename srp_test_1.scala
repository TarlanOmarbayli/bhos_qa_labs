...
class SRPGatlingTestSimulation extends Simulation {
  ...
  val scn = scenario("SRP Gatling Test Simulation").exec(http("SRPAuthRequest").get("/srpLoadTest"))
  setUp(scn.inject(atOnceUsers(5), constantUsersPerSec(25).during(30.minutes),).protocols(httpProtocol))
}